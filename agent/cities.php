                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class=" d-flex justify-content-between">
                                        <h3 style="margin-top:20px;margin-left:20px"  class="card-title">Cities</h3>
                                        <button  style="margin-top:20px;margin-right:20px" class="btn btn-success add-feature">Add City</button>
                                    </div>
                                    <!-- /.card-header -->
                                    <div class="card-body">
      ...
      <div class="modal-body">
        <form>
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="feature-title">
          </div>
        </form>
      </div>
...
<script>
 ...
 jQuery(document).on('click', '.store-feature', function(event) {
    var formData = new FormData();
            formData.append('name',$('#feature-title').val());
    axios.post('<?php echo $host_url; ?>/city', formData, {
                headers: {
                ...
                }).then((res)=>
                {
                    ...
                    text: "Your City Has Been Added Successfully !",
                    type: "success"
                    }).then(function() {
 ...
 jQuery(document).on('click', '.update-feature', function(event) {
    ...
    axios.post('<?php echo $host_url; ?>/city/'+$('#feature-id').val()+"/update", formData, {
                headers: {
                    "Content-Type": "multipart/form-data",
                    "Accept" : "application/json"
                },
                ...
                {
                    swal({
                    title: "City Edited Successful !",
                    text: "Your City Has Been Updated Successfully !",
                    type: "success"
                    }).then(function() {
      ...
      'serverMethod': 'get',
      'ajax': {
          'url':'<?php echo $host_url; ?>/cities/datatable'
      },
      'columns': [
        {data:'id'},
         { data: 'name' },
      ...
      {  targets: 2,
         render: function (data, type, row, meta) {
            return '<button class="btn btn-primary btn-small edit-feature" data-id="'+data+'" data-title="'+row.name+'">Edit</button> <button class="btn btn-danger btn-small" onClick="deleteFeature('+data+')">Delete</button>';
         }
    ...
    function deleteFeature(id)
        ...
        swal({
            ...
            .then((willDelete) => {
            if (willDelete) {
                axios.delete('<?php echo $host_url; ?>/city/'+id+'/delete').then((res)=>{

                    if(res.data == 'fail')
                    {
                        
                    swal("Your City Cannot be deleted Property Assigned !!", {
                        icon: "error",
                        });
                    }
                    else
                    {
                        swal("Your City Is Deleted Successfully !!", {
                        icon: "success",
                        });
